...
int main()
{
    ...
    ifstream in("superman_black.pgm" , ios::binary);
    string type , comment;
    ...
    unsigned char *p = new unsigned char[width*height];
    in.read( (char *) p , width * height);
    in.close();


    //Perform Operations : (either on 1D or 2D array)
    //While performing operations of read or write try to use unsigned char 


    //Now writing the data :
    ofstream out("superman_black_new.pgm",ios::binary);
    out<<type<<"\n";
    out<<comment<<"\n";
    out<<newWidth<<" "<<newHeight<<"\n";
    out<<maxval<<"\n";
    out.write( (char *)p,  width*height);
